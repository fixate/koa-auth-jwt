...
function init(options) {
  const secretOrPublicKey = options.publicKey || options.secret;
  const algorithm = options.publicKey ? 'RS256' : 'HS256';
  ...
  function verifyToken(token, validation) {
    return new Promise((resolve, reject) => {
      ...
      jwt.verify(token, secretOrPublicKey, { algorithms: [algorithm] }, (err, decoded) => {
        if (err) {
